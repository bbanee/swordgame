class SeasonPassReward {
  ...
  final String?
  specialRewardType; // 'gacha_ticket', 'unique_sword', 'legend_chance'
  ...
  // 프리미엄 보상 있는지
  bool get hasPremiumReward =>
      premiumGold > 0 || premiumDiamond > 0 || premiumStone > 0;
}
...
List<SeasonPassReward> getSeasonPassRewards({required int maxLevel}) {
  ...
  for (int lv = 1; lv <= maxLevel; lv++) {
    ...
    String? specialType;
    if (lv == 10)
      specialType = 'premium_gacha_5x';
    else if (lv == 20)
      specialType = 'unique_selector';
    else if (lv == 30)
      specialType = 'legend_ticket';
    else if (lv == 40)
      specialType = 'premium_gacha_10x';
    else if (lv == 50)
      specialType = 'legend_selector';
