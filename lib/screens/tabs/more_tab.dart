...
class MoreTab extends StatelessWidget {
    ...
    return ListView(
      ...
      children: [
        ...
        const Padding(
          ...
          child: Text('🎬 광고 보상', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        
        _buildAdRewardItem(
          '💎', '무료 고급 뽑기', '광고 보고 다이아 뽑기 1회!',
          adService.getRemainingAdCount(AdRewardType.freeGacha), 1,
          adService.canWatchAd(AdRewardType.freeGacha), onWatchAdForFreeGacha,
        ),
        
        _buildAdRewardItem(
          '🪨', '강화석 획득', '광고 보고 강화석 3개 획득!',
          adService.getRemainingAdCount(AdRewardType.stoneReward), 3,
          adService.canWatchAd(AdRewardType.stoneReward), onWatchAdForStones,
        ),
        ...
        _buildMoreItem('🌐', '공식 웹사이트', '공지사항 및 업데이트 정보', () => _launchUrl('https://www.opentheday.site/')),
        ...
        _buildMoreItem('❌', '계정 삭제', '계정 및 모든 데이터 삭제', onShowDeleteAccountDialog),
      ],
    );
  }
  ...
  Widget _buildMoreItem(String icon, String title, String sub, VoidCallback onTap) {
  ...
  Widget _buildAdRewardItem(String icon, String title, String sub, int remaining, int max, bool canWatch, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: canWatch ? LinearGradient(colors: [Colors.green.withOpacity(0.2), Colors.blue.withOpacity(0.2)]) : null,
          color: canWatch ? null : Colors.grey.withOpacity(0.1),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: canWatch ? Colors.green.withOpacity(0.5) : Colors.grey.withOpacity(0.3)),
        ),
        child: Row(children: [
          Text(icon, style: const TextStyle(fontSize: 28)),
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: TextStyle(color: canWatch ? Colors.white : Colors.grey, fontWeight: FontWeight.bold)),
            Text(sub, style: TextStyle(color: canWatch ? Colors.white54 : Colors.grey, fontSize: 12)),
          ])),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(color: canWatch ? Colors.green : Colors.grey, borderRadius: BorderRadius.circular(16)),
            child: Text(canWatch ? '🎬 $remaining/$max' : '완료', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        ]),
      ),
    );
