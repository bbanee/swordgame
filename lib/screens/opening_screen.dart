...
class _OpeningScreenState extends State<OpeningScreen>
  ...
  Future<void> _loadExistingData() async {
    try {
      ...
      setState(() {
        _existingNickname = (nickname != null && nickname.isNotEmpty)
            ? nickname
            : null;
      ...
      debugPrint(
        '📱 기존 데이터 로드: nickname=$_existingNickname, uid=$_existingUserId',
      );
    } catch (e) {
  ...
  Future<void> _validateAndStart() async {
    ...
    if (nickname.length < 2) {
      setState(() => _errorMessage = '2자 이상 입력해주세요');
      return;
    }
    if (nickname.length > 10) {
      setState(() => _errorMessage = '10자 이하로 입력해주세요');
      return;
    }

    final validPattern = RegExp(r'^[가-힣a-zA-Z0-9]+$');
    if (!validPattern.hasMatch(nickname)) {
      setState(() => _errorMessage = '한글, 영문, 숫자만 사용 가능합니다');
      return;
    }

    final bannedWords = ['관리자', 'admin', 'gm', 'GM', '운영자'];
    if (bannedWords.any(
      (word) => nickname.toLowerCase().contains(word.toLowerCase()),
    )) {
      setState(() => _errorMessage = '사용할 수 없는 닉네임입니다');
      return;
    }
  ...
  Future<void> _resetAndStartNewGame(String nickname) async {
    try {
      ...
      setState(() {
        _isProcessing = false;
        _errorMessage = '초기화 중 오류가 발생했습니다. 다시 시도해주세요.';
      });
    }
  }
  ...
  Future<void> _updateNicknameAndStart(String nickname) async {
    try {
      ...
      setState(() {
        _isProcessing = false;
        _errorMessage = '저장 중 오류가 발생했습니다. 다시 시도해주세요.';
      });
    }
  }
    ...
    Navigator.pushReplacement(
      ...
      PageRouteBuilder(
        pageBuilder: (_, __, ___) =>
            GameScreen(nickname: nickname, userId: userId),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // 새로 시작 / 게임 시작 버튼
          _buildMenuCard(
            icon: _existingNickname != null
                ? Icons.refresh_rounded
                : Icons.star_rounded,
            title: _existingNickname != null ? '새로 시작' : '게임 시작',
            subtitle: _existingNickname != null
                ? '모든 데이터가 초기화됩니다'
                : '새로운 모험을 떠나세요',
      ...
      builder: (_, __) => Transform.translate(
        ...
        child: Container(
          ...
          child: Image.asset(
            ...
            fit: BoxFit.contain,
            errorBuilder: (_, __, ___) =>
                const Icon(Icons.shield, size: 70, color: Colors.amber),
          ),
        ),
      ),
    );
        ...
        children: [
            ...
            builder: (_, __) => Transform.translate(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: (_isNewGame ? Colors.green : Colors.indigo)
                          .withOpacity(0.15 * _swordGlow.value),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: (_isNewGame ? Colors.green : Colors.indigo).withOpacity(
                  0.2,
                ),
              ),
              boxShadow: [
                  ...
                  decoration: InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      ...
                      borderSide: BorderSide(
                        color: (_isNewGame ? Colors.green : Colors.amber)
                            .withOpacity(0.6),
                        width: 1.5,
                      ),
                    ),
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color:
                                    (_isNewGame ? Colors.green : Colors.amber)
                                        .withOpacity(0.3),
                                blurRadius: 10,
